...
<div class="container_wrapper">
    ...
    <div class="home_content">
		...
		$line = 0;
		$page = 'Feedbacks.php';
		$dataperpage = $connect -> query("SELECT * FROM tblfeedback");
		...
		$feedbacks = $connect -> query("SELECT * FROM tblfeedback LIMIT $eu,$limit");
		while($row = $feedbacks -> fetch_array(MYSQLI_ASSOC)){
			if($row['status']==1){
				$fontcolor = '#FF3C3C';	
			}
			if($line == 1){
